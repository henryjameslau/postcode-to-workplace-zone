...
if(Modernizr.webgl) {
	...
	d3.queue()
		...
		.defer(d3.json, "data/workplacezoneUK.json")
		.await(ready);
	...
	// };
		...
		function selectlist(datacsv) {
			...
			var areacodes =  datacsv.map(function(d) { return d["Group Code"]; }).filter(function(d,i,arr){return arr.indexOf(d)==i});
			var areanames =  datacsv.map(function(d) { return d["Group Name"];}).filter(function(d,i,arr){return arr.indexOf(d)==i});
			...
			myId=null;
			 ...
			 $('#areaselect').chosen({placeholder_text_single:"Select a workplace zone group",allow_single_deselect:true})
	     ...
	     d3.select('div.chosen-search').insert('label','input.chosen-search-input').attr('class','visuallyhidden').attr('for','chosensearchinput').html("Type to select an area")
			...
			$('#areaselect').on('change',function(){
					...
					if($('#areaselect').val() != "") {
							...
							map.setFilter("area", ["==", "Group Code", areacode]);
							map.setFilter("state-fills-hover", ["==", "Group Code", areacode]);
							// selectArea(areacode);
							// setAxisVal(areacode);
							// zoomToArea(areacode);
							//
              ...
              // })
							...
							// hideaxisVal();
							// onLeave();
							map.setFilter("area", ["==", "Group Code", ""]);
							map.setFilter("state-fills-hover", ["==", "Group Code", ""]);
							resetZoom();
					}
			});
	};//end selectlist
